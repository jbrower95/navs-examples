 ...
 * 
 * Generated by navs-gen on 2025-06-10T05:43:02.856Z
 * Total functions: 1
 */
abstract contract NavsReceiver is IBaseNavsReceiver {

    // Package information constants
    string public constant PACKAGE_NAME = "navs-launchpad";
    ...
    function _handleNavsCallback(
        ...
        if (keccak256(bytes(functionName)) == keccak256(bytes("getTokenAllocation"))) {
            ...
            uint256 resultValue = abi.decode(resultBytes, (uint256));
            
            onGetTokenAllocation(taskId, param0, resultValue, error);
            return;
        }
    ...
    function _handleNavsErrorCallback(
        ...
        if (keccak256(bytes(functionName)) == keccak256(bytes("getTokenAllocation"))) {
            onGetTokenAllocation(taskId, address(0), 0, error);
            return;
        }
    ...
    /**
     * Call getTokenAllocation with callback
     * Function getTokenAllocation from index.ts
     ...
     */
    function getTokenAllocation(address address_param, uint256 requiredStakeWei) internal returns (uint256 taskId) {
    ...
    function _submitTaskWithCallback(
        ...
        uint256 taskId = ITaskDispatch(taskDispatchAddr).submitTask(
            "navs-launchpad", // Service name from package.json
            "1.0.3", // Service version from package.json
            functionName,
            encodedParams,
    ...
    /**
     * Abstract callback for getTokenAllocation function completion
     * Override this function in your contract to handle getTokenAllocation results
     * @param taskId The unique identifier for the completed task
     * @param address_param Original parameter of type address
     * @param result The result returned by getTokenAllocation
     * @param error Error message if the function failed, empty string if successful
     */
    function onGetTokenAllocation(uint256 taskId, address address_param, uint256 result, string memory error) internal virtual;
}
